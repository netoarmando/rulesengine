...
class ViewsTestCase(TestCase):
    ...
    def test_rules_get_surt_exact(self):
        response = self.client.get("/rules", {"surt-exact": "https://(org,archive,"})
    ...
    def test_rules_get_surt_starts_with(self):
        response = self.client.get("/rules", {"surt-start": "https://(org,archive,"})
        self.assertEqual(response.status_code, 200)
        parsed = json.loads(response.content.decode("utf-8"))
        self.assertEqual(parsed["status"], "success")
        self.assertEqual(parsed["message"], "ok")
        for result in parsed["result"]:
            self.assertEqual(result["surt"].startswith("https://(org,archive,"), True)
    ...
    def test_rules_new_success(self):
        ...
        self.assertEqual(parsed["status"], "success")
        self.assertEqual(parsed["message"], "ok")
        new_rule = Rule.objects.get(surt="http://(doctor,who,)/companions/rose")
    ...
    def test_rules_new_fail_validation(self):
        response = self.client.post(
            "/rules", content_type="application/json", data=json.dumps({"bad": "wolf"})
        )
        self.assertEqual(response.status_code, 400)
        parsed = json.loads(response.content.decode("utf-8"))
        self.assertEqual(parsed["status"], "error")
    ...
    def test_rule_update_success(self):
        ...
        response2 = self.client.put(
            "/rule/1", content_type="application/json", data=json.dumps(payload)
        )
        self.assertEqual(response2.status_code, 200)
        update = json.loads(response2.content.decode("utf-8"))
        self.assertEqual(update["result"]["rule"]["public_comment"], "UPDATE")
        self.assertEqual(update["result"]["change"]["rule"], parsed["result"])
    ...
    def test_rule_update_fail_validation(self):
        response = self.client.put(
            "/rule/1", content_type="application/json", data=json.dumps({"bad": "wolf"})
        )
        self.assertEqual(response.status_code, 400)
        parsed = json.loads(response.content.decode("utf-8"))
        self.assertEqual(parsed["status"], "error")
    ...
    def test_rules_for_request_missing_params(self):
        ...
        parsed = json.loads(response.content.decode("utf-8"))
        self.assertEqual(parsed["status"], "error")
        self.assertEqual(parsed["message"], "surt query string param is required")
